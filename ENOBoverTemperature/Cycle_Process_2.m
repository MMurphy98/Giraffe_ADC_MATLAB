...
%       siand/thd_with/withoutcalib: dynamic specifications of signal;
    load(filename, 'data_N4_ad', 'sinad_withcalib', 'thd_withcalib', 'data_withcalib');
    Vref = 1.8;
    N_SUB = 5;
...
%% With Const Gain calibration
    ...
    Cycle.Vamp = 0.5*(max(data_withcalib) - min(data_withcalib));
    Cycle.Vamp_const_gain = 0.5*(max(Vout_const_gain) - min(Vout_const_gain));
%% Sweep Gain 
